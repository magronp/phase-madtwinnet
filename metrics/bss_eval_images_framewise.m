function [SDR,ISR,SIR,SAR]=bss_eval_images_framewise(se,sm)

Fs = 44100;
[J,L]=size(sm);
window = 30*Fs; hop = 15*Fs;

nwin = floor( (L-window + hop) / hop);

SDR = zeros(J,nwin); ISR = zeros(J,nwin); SIR = zeros(J,nwin); SAR = zeros(J,nwin);

for k=1:nwin
    win_slice = ((k-1) * hop+1) : ((k-1) * hop + window);
    se_slice = se(:,win_slice);
    sm_slice = sm(:,win_slice);
    [sdr,isr,sir,sar] = bss_eval_images(se_slice,sm_slice);
    SDR(:,k) = sdr; ISR(:,k) = isr; SIR(:,k) = sir; SAR(:,k) = sar;
end
